...
def getLetters():
    ...
    for imgList in imgarr:
        for camNum in range(0,3):
            ...
            """
            if camNum == 0:
                cv2.imwrite(str(u)+" left.png", thresh)
            if camNum == 1:
                cv2.imwrite(str(u)+" center.png", thresh)
            if camNum == 2:
                cv2.imwrite(str(u)+" right.png", thresh)
            """
            ...
            for i, c in enumerate(contours):
                ...
                if(cv2.contourArea(c)>65):
                    ...
                    right = left
                    #cropped = cv2.copyMakeBorder(cropped, top, bottom, left, right, cv2.BORDER_CONSTANT, np.array([0, 0, 0]))
                    cropped = cv2.GaussianBlur(cropped,(3, 3), 0)
                    cropped = cv2.cvtColor(cropped, cv2.COLOR_RGB2HSV)
                    cropped = cv2.inRange(cropped, np.array([0, 0, 189]), np.array([179, 255, 255]))
                    #cropped = cv2.cvtColor(cropped, cv2.COLOR_HSV2GRAY)
